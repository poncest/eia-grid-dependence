...
|---|---|---|---|
| Tacoma Power (TPWR) | Net import dependence increased | BPAT (89.2% of attributed contribution) | Robust |
...
"Robust" is a fixed, pre-set rule (script 10). The finding must hold in the baseline top-5% comparison, in all three reconciliation scenarios, at all three stress thresholds (1%, 5% and 10%), and when partially observed flows are included. Named extreme-weather windows are also tested, but as an informational flag (`EVENT_VARIABLE`) rather than a pass/fail gate, because three short event windows are a much smaller sample than a percentile definition. The leading BPAT and SWPP corridors also remained #1 across all reconciliation scenarios. Other candidates were investigated and reported honestly in the methodology page, including GCPD, a real flip that did not survive alternative stress thresholds and so is not a headline claim.
...
The Esri layer does not state its vintage, so it is **not** treated as a confirmed 2025 EIA snapshot. It affects only where nodes sit on the schematic map, not any analytical result.
...
- **Event windows are short.** The three named extreme-weather windows are small samples, so disagreement across them is flagged, not treated as disqualifying.
...
- **Geographic sources:** State boundaries come from the `maps` R package. BA anchors were derived from the Esri Policy Maps "Balancing Authority Energy Summary" layer, whose reuse terms are _TODO: copy the Terms of Use and Credits from the layer's item page_. This repository does not redistribute that geometry (`data/` is not tracked).
- **Code and written material:** _TODO: choose a license and add a matching `LICENSE` file (for example MIT for code, CC BY 4.0 for the figures and prose)._
